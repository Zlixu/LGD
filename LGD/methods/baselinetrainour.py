    ...
    def train_loop(self, epoch, base_loader, optimizer, params=None):
        ...
        for i, (x, y) in enumerate(train_loader):
            if params.ts_align or params.st_align:
                ...
                softmax_outputs_test = torch.nn.Softmax(dim=1)(self.classifier(fx1))
                alpha = 0.3
                lam = np.random.beta(alpha, alpha)
                index = torch.randperm(x1.size()[0]).cuda()
                mixed_input = lam *x1 + (1 - lam) * x1[index, :]
                #print("mixx:",mixed_input.shape)
                # mixed_input.Size([64, 3, 224, 224])
                #print(mixed_input.size())

                mixed_output = (lam * softmax_outputs_test + (1 - lam) * softmax_outputs_test[index, :]).detach()
    
                #update_batch_stats(model, False)
                outputs_target_m = self.classifier(self.feature(mixed_input))
                #update_batch_stats(model, True)
                outputs_target_m = torch.nn.Softmax(dim=1)(outputs_target_m)
                classifier_loss = 0.8*nn.KLDivLoss(reduction='batchmean')(outputs_target_m.log(), mixed_output)+0.6 * 0.8*nn.KLDivLoss(reduction='batchmean')(mixed_output.log(), outputs_target_m)


                #mixuptarget
                softmax_outputs_testux = torch.nn.Softmax(dim=1)(self.classifier(self.feature(ux1)))
                alpha = 0.3
                lam = np.random.beta(alpha, alpha)
                index = torch.randperm(ux1.size()[0]).cuda()
                mixed_inputux = lam *ux1 + (1 - lam) * ux1[index, :]
                #print("mixux:",mixed_inputux.shape)
                # mixed_input.Size([64, 3, 224, 224])
                #print(mixed_input.size())
                mixed_outputux = (lam * softmax_outputs_testux + (1 - lam) * softmax_outputs_testux[index, :]).detach()
                #update_batch_stats(model, False)
                outputs_target_mux = self.classifier(self.feature(mixed_inputux))
                #update_batch_stats(model, True)
                outputs_target_mux = torch.nn.Softmax(dim=1)(outputs_target_mux)
                classifier_lossux = 0.8*nn.KLDivLoss(reduction='batchmean')(outputs_target_mux.log(), mixed_outputux)+ 0.6 * 0.8*nn.KLDivLoss(reduction='batchmean')(mixed_outputux.log(), outputs_target_mux)
                classifier_loss = classifier_loss + classifier_lossux 
                ...
                lam = np.random.beta(alpha, alpha)
                index = torch.randperm(x1.size()[0]).cuda()
                mixed_input_xux = lam *x1 + (1 - lam) * ux1[index, :]
                #print("mixx:",mixed_input.shape)
                # mixed_input.Size([64, 3, 224, 224])
                #print(mixed_input.size())

                mixed_output_xux = (lam * softmax_outputs_test + (1 - lam) * softmax_outputs_testux[index, :]).detach()
    
                #update_batch_stats(model, False)
                outputs_target_m_xux = self.classifier(self.feature(mixed_input_xux))
                #update_batch_stats(model, True)
                outputs_target_m_xux = torch.nn.Softmax(dim=1)(outputs_target_m_xux)
                classifier_loss_xux = 0.8*nn.KLDivLoss(reduction='batchmean')(outputs_target_m_xux.log(), mixed_output_xux)+0.6 * 0.8*nn.KLDivLoss(reduction='batchmean')(mixed_output_xux.log(), outputs_target_m_xux)
                classifier_loss = classifier_loss + classifier_loss_xux 



                # classification head for labeled images.
                #print("self.classifier(fx1):",self.classifier(fx1).shape)
                #print("y:",y.shape)
                loss = self.loss_fn(self.classifier(fx1), y)+classifier_loss
            ...
            if i % print_freq == 0:
                print_line = 'Epoch {:d} | Batch {:d}/{:d} | Loss {:f} | Classifier_loss {:f} '.format(epoch, i, len(train_loader), avg_loss / float(i + 1),classifier_loss)
